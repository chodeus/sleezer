...
namespace NzbDrone.Plugin.Sleezer.Metadata.FFmpeg
{
    ...
    public class FFmpegSettings : IProviderConfig
    {
        ...
        [FieldDefinition(10, Label = "Run Corrupt Scan On", Type = FieldType.TagSelect, SelectOptions = typeof(PostProcessClient), Section = MetadataSectionType.Metadata, Placeholder = "Type d, t, s, or q to add a client", HelpText = "After download, scan audio files for corruption (size, TagLib parse, ffmpeg decode) on the selected Sleezer downloaders. Corrupt files are quarantined and the release is re-searched. Empty = scan disabled. Does not run on torrent/Usenet or other web clients.")]
        public IEnumerable<int> CorruptionScanClients { get; set; } = Array.Empty<int>();

        [FieldDefinition(11, Label = "Run Pre-Import Tagging On", Type = FieldType.TagSelect, SelectOptions = typeof(PostProcessClient), Section = MetadataSectionType.Metadata, Placeholder = "Type d, t, s, or q to add a client", HelpText = "Run Lidarr's identification + tag writer on downloaded files before import for the selected Sleezer downloaders, so untagged or mistagged releases match cleanly. Empty = tagging disabled. Does not run on torrent/Usenet or other web clients.")]
