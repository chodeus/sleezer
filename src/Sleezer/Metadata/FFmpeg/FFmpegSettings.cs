...
namespace NzbDrone.Plugin.Sleezer.Metadata.FFmpeg
{
    ...
    public class FFmpegSettings : IProviderConfig
    {
        ...
        [FieldDefinition(10, Label = "Run Corrupt Scan On", Type = FieldType.TagSelect, SelectOptions = typeof(PostProcessClient), Section = MetadataSectionType.Metadata, Placeholder = "Type to add a client", HelpText = "After download, scan audio files for corruption (size, TagLib parse, ffmpeg decode) on the selected Sleezer downloaders. Corrupt files are quarantined and the release is re-searched. Empty = scan disabled. Does not run on torrent or Usenet downloads.")]
