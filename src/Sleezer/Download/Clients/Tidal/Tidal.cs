...
namespace NzbDrone.Core.Download.Clients.Tidal
{
    public class Tidal : DownloadClientBase<TidalSettings>
    {
        ...
        // Public-static so DownloadItem's failed-download cleanup can reuse it.
        internal static void TryRemoveEmptyParentFolders(string startedAt, string downloadRoot, Logger logger)
        {
            try
            {
                ...
                while (!string.IsNullOrEmpty(current))
                {
                    ...
                    try
                    {
                        ...
                        logger.Debug("Tidal: removed empty parent folder {Folder}", current);
                    }
                    catch (Exception ex)
                    {
                        logger.Debug(ex, "Tidal: could not remove empty parent {Folder}; stopping sweep", current);
                        return;
                    }
            ...
            {
                logger.Debug(ex, "Tidal: empty-parent sweep aborted from {Start}", startedAt);
            }
        }
