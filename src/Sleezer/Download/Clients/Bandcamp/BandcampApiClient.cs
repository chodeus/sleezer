...
namespace NzbDrone.Core.Download.Clients.Bandcamp
{
    ...
    public class BandcampApiClient
    {
        ...
        // ParseSizeBytes is static; the instance logger is not reachable from it.
        ...
        private static long ParseSizeBytes(JsonElement sizeMbEl)
        {
            ...
            {
                // A malformed size is not worth failing a grab over; 0 means "unknown".
                _staticLogger.Trace(ex, "Bandcamp API: could not parse a download size, treating it as unknown");
            }
