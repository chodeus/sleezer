...
namespace NzbDrone.Core.Download.Clients.Bandcamp
{
    ...
    public class BandcampDownloadProxy : IBandcampDownloadProxy
    {
        ...
        // Static helpers reachable from internal static entry points (the retag path
        // the tests drive directly) cannot see the instance logger.
        ...
        private static AudioTag? SafeReadAudioTag(string path)
        {
            ...
            {
                _staticLogger.Trace(ex, "Bandcamp download proxy: could not read tags from {File}", path);
                return null;
            }
        }
        ...
        private bool IsZipFile(string path)
        {
            ...
            {
                _logger.Trace(ex, "Bandcamp download proxy: could not probe {File} for a zip header", path);
                return false;
            }
        }
        ...
        /// </summary>
        private string? DetectFileExtension(string path)
        {
            try
            {
                using var stream = File.OpenRead(path);
            ...
            {
                _logger.Trace(ex, "Bandcamp download proxy: could not sniff the format of {File}", path);
                return null;
            }
        }
        ...
        private void NormalizeExtractedPermissions(string outputDir)
        {
            ...
            {
                // Best-effort only; the real extraction/import error should surface instead.
                _logger.Trace(ex, "Bandcamp download proxy: permission normalization failed under {Dir}", outputDir);
            }
        }

        private void NormalizeDirectoryPermissions(string path)
        {
            if (OperatingSystem.IsWindows())
            {
                return;
            ...
            {
                _logger.Trace(ex, "Bandcamp download proxy: could not set directory mode on {Dir}", path);
            }
        }

        private void NormalizeFilePermissions(string path)
        {
            if (OperatingSystem.IsWindows())
            {
                return;
            ...
            {
                _logger.Trace(ex, "Bandcamp download proxy: could not set file mode on {File}", path);
            }
        }
        ...
        private void TryDeleteFile(string path)
        {
            ...
            {
                // Never mask the original exception this cleanup is running under.
                _logger.Trace(ex, "Bandcamp download proxy: could not delete temp file {File}", path);
            }
        }
    }
}
